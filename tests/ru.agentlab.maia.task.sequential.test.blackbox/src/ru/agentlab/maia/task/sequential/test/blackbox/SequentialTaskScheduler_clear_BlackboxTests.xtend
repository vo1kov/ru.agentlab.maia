...
import ru.agentlab.maia.task.test.blackbox.TaskScheduler_clear_Steps

@RunWith(JUnitReportingRunner)
class SequentialTaskScheduler_clear_BlackboxTests extends JUnitStories {
	...
	override stepsFactory() {
		...
		return new InstanceStepsFactory(
			configuration,
//			new SequentialTaskSchedulerSteps(provider),
			new TaskScheduler_clear_Steps(provider)
		)
	}
...
//		val codeLocation = CodeLocations.codeLocationFromClass(this.getClass()).getFile();
