...
import ru.agentlab.maia.task.test.blackbox.TaskScheduler_removeSubtask_Steps

@RunWith(JUnitReportingRunner)
class SequentialTaskScheduler_removeSubtask_BlackboxTests extends JUnitStories {
	...
	override stepsFactory() {
		...
		return new InstanceStepsFactory(
			configuration,
			new TaskScheduler_removeSubtask_Steps(provider)
		)
	}
