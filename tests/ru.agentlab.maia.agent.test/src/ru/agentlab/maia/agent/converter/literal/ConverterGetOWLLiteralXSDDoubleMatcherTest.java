...
import static ru.agentlab.maia.agent.converter.MatcherUtil._dou;
import static ru.agentlab.maia.agent.converter.MatcherUtil._var;
import static ru.agentlab.maia.agent.converter.MatcherUtil.doublMatcher;
...
public class ConverterGetOWLLiteralXSDDoubleMatcherTest extends AbstractGetOWLLiteralMatcherTest {
	...
	public static Collection<Object[]> data() {
		return Arrays.asList(new Object[][] {
			...
			/*  1 */ 	{ "0^^xsd:double", 						doublMatcher(_dou(0)) },						// test 0 string
			/*  2 */ 	{ "-0^^xsd:double", 					doublMatcher(_dou(0)) },						// test 0 string
			/*  3 */ 	{ "+0^^xsd:double", 					doublMatcher(_dou(0)) },						// test 0 string
			/*  4 */ 	{ "1^^xsd:double", 						doublMatcher(_dou(1)) },						// test 1 string
			/*  5 */ 	{ "-1^^xsd:double", 					doublMatcher(_dou(-1)) },						// test 1 string
			/*  6 */ 	{ "+1^^xsd:double", 					doublMatcher(_dou(1)) },						// test 1 string
			/*  7 */ 	{ "12345^^<" + XSD + "double>",			doublMatcher(_dou(12345)) }, 					// test more numbers
			/*  8 */ 	{ "2.2E12^^<" + XSD + "double>",		doublMatcher(_dou(2.2e12)) }, 						// test dot name
			/*  9 */ 	{ Double.MAX_VALUE+"^^xsd:double",		doublMatcher(_dou(Double.MAX_VALUE)) }, 		// test max value
			/* 10 */ 	{ "-"+Double.MAX_VALUE+"^^xsd:double",	doublMatcher(_dou(-Double.MAX_VALUE)) }, 		// test min value
			/* 11 */ 	{ Double.MAX_VALUE+"0^^xsd:double",		doublMatcher(_dou(Double.POSITIVE_INFINITY)) }, // test > +infinity
			/* 12 */ 	{ "INF^^xsd:double",					doublMatcher(_dou(Double.POSITIVE_INFINITY)) }, // test +infinity
			/* 13 */ 	{ "-"+Double.MAX_VALUE+"0^^xsd:double",	doublMatcher(_dou(Double.NEGATIVE_INFINITY)) }, // test < -infinity
			/* 14 */ 	{ "-INF^^xsd:double",					doublMatcher(_dou(Double.NEGATIVE_INFINITY)) }, // test +infinity
			/* 15 */ 	{ "NaN^^xsd:double",					doublMatcher(_dou(Double.NaN)) }, 				// test NaN
